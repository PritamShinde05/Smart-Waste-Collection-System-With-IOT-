...
function AdminRoutes() {
    ...
    const fetchBins = async () => {
        try {
            ...
            setMessage("Failed to fetch smart bins");
        }
    };
    ...
    const generateRoute = async (e) => {
        ...
        try {
            const url = `${API_BASE_URL}/routes/admin/task/${formData.task_id}?start_latitude=${formData.start_latitude}&start_longitude=${formData.start_longitude}`;
                            ...
                            >
                                ...
                                {tasks.map((task) => (
                                    <option key={task.task_id} value={task.task_id}>
                                        Task #{task.task_id} - {task.employee_name} - {task.area_name}
                                    </option>
                                ))}
                            </select>
                        </div>
            ...
            {routeData && (
                <>
                    <div className="table-container">
                        <h3>Route Map</h3>
                        <p>
                            Red marker = Starting point, Blue markers = Route bins,
                            Green markers = Other smart bins.
                        </p>

                        <RouteMap routeData={routeData} allBins={allBins} />
                    </div>

                    <div className="table-container">
                        <h3>Optimized Route Result</h3>
